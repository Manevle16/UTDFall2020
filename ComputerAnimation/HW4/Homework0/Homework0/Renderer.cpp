...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addButton("Start Animation", []() {
		m_aircraft_animation->isMoving = true;
	});

	static auto animation_t1 = gui_1->addVariable("t1", m_aircraft_animation->t1);
	static auto animation_t2 = gui_1->addVariable("t2", m_aircraft_animation->t2);
	static auto animation_total_time = gui_1->addVariable("total moving time", m_aircraft_animation->total_moving_time);

	gui_1->addButton("Reset Movement", []() {
		m_aircraft_animation->reset();
		animation_t1->setValue(m_aircraft_animation->t1);
		animation_t2->setValue(m_aircraft_animation->t2);
		animation_total_time->setValue(m_aircraft_animation->total_moving_time);
	});
