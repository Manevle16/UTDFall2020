...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	static auto rotation_x1_widget = gui_1->addVariable("Rotate X1", m_bone_animation->rotation_degree_vector[1].x);
	static auto rotation_y1_widget = gui_1->addVariable("Rotate Y1", m_bone_animation->rotation_degree_vector[1].y);
	static auto rotation_z1_widget = gui_1->addVariable("Rotate Z1", m_bone_animation->rotation_degree_vector[1].z);

	static auto rotation_x2_widget = gui_1->addVariable("Rotate X2", m_bone_animation->rotation_degree_vector[2].x);
	static auto rotation_y2_widget = gui_1->addVariable("Rotate Y2", m_bone_animation->rotation_degree_vector[2].y);
	static auto rotation_z2_widget = gui_1->addVariable("Rotate Z2", m_bone_animation->rotation_degree_vector[2].z);

	static auto rotation_x3_widget = gui_1->addVariable("Rotate X3", m_bone_animation->rotation_degree_vector[3].x);
	static auto rotation_y3_widget = gui_1->addVariable("Rotate Y3", m_bone_animation->rotation_degree_vector[3].y);
	static auto rotation_z3_widget = gui_1->addVariable("Rotate Z3", m_bone_animation->rotation_degree_vector[3].z);

	gui_1->addButton("Reset Bone Rotation", []() {
		m_bone_animation->reset();
		rotation_x1_widget->setValue(m_bone_animation->rotation_degree_vector[1].x);
		rotation_y1_widget->setValue(m_bone_animation->rotation_degree_vector[1].y);
		rotation_z1_widget->setValue(m_bone_animation->rotation_degree_vector[1].z);

		rotation_x2_widget->setValue(m_bone_animation->rotation_degree_vector[2].x);
		rotation_y2_widget->setValue(m_bone_animation->rotation_degree_vector[2].y);
		rotation_z2_widget->setValue(m_bone_animation->rotation_degree_vector[2].z);

		rotation_x3_widget->setValue(m_bone_animation->rotation_degree_vector[3].x);
		rotation_y3_widget->setValue(m_bone_animation->rotation_degree_vector[3].y);
		rotation_z3_widget->setValue(m_bone_animation->rotation_degree_vector[3].z);
	});
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
	...
	root_bone_obj_mat = glm::translate(root_bone_obj_mat, m_bone_animation->position_vector[0]);
	...
	//Initialize Bones
	glm::mat4 blue_bone_obj_mat = glm::mat4(1.0f);
	glm::mat4 pink_bone_obj_mat = glm::mat4(1.0f);
	glm::mat4 yellow_bone_obj_mat = glm::mat4(1.0f);

	//Translate Bones onto root
	yellow_bone_obj_mat = glm::translate(yellow_bone_obj_mat, m_bone_animation->position_vector[1]);
	pink_bone_obj_mat = glm::translate(pink_bone_obj_mat, m_bone_animation->position_vector[1]);
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, m_bone_animation->position_vector[1]);

	//Rotate Yellow Bone Link    
	pink_bone_obj_mat = glm::translate(pink_bone_obj_mat, glm::vec3({ 0.0f, -2.0f, 0.0f }));
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, glm::vec3({ 0.0f, -2.0f, 0.0f }));
	yellow_bone_obj_mat = glm::translate(yellow_bone_obj_mat, glm::vec3({ 0.0f, -2.0f, 0.0f }));
	pink_bone_obj_mat = m_bone_animation->rotate_bone(pink_bone_obj_mat, m_bone_animation->rotation_degree_vector[1]);
	blue_bone_obj_mat = m_bone_animation->rotate_bone(blue_bone_obj_mat, m_bone_animation->rotation_degree_vector[1]);
	yellow_bone_obj_mat = m_bone_animation->rotate_bone(yellow_bone_obj_mat, m_bone_animation->rotation_degree_vector[1]);
	pink_bone_obj_mat = glm::translate(pink_bone_obj_mat, glm::vec3({ 0.0f, 2.0f, 0.0f }));
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, glm::vec3({ 0.0f, 2.0f, 0.0f }));
	yellow_bone_obj_mat = glm::translate(yellow_bone_obj_mat, glm::vec3({ 0.0f, 2.0f, 0.0f }));
	...
	//Rotate Pink Bone Link
	pink_bone_obj_mat = glm::translate(pink_bone_obj_mat, glm::vec3({ 0.0f, 3.5f, 0.0f }));
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, glm::vec3({ 0.0f, 3.5f, 0.0f }));
	pink_bone_obj_mat = glm::translate(pink_bone_obj_mat, glm::vec3({ 0.0f, -1.5f, 0.0f }));
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, glm::vec3({ 0.0f, -1.5f, 0.0f }));
	pink_bone_obj_mat = m_bone_animation->rotate_bone(pink_bone_obj_mat, m_bone_animation->rotation_degree_vector[2]);
	blue_bone_obj_mat = m_bone_animation->rotate_bone(blue_bone_obj_mat, m_bone_animation->rotation_degree_vector[2]);
	pink_bone_obj_mat = glm::translate(pink_bone_obj_mat, glm::vec3({ 0.0f, 1.5f, 0.0f }));
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, glm::vec3({ 0.0f, 1.5f, 0.0f }));
	pink_bone_obj_mat = glm::scale(pink_bone_obj_mat, m_bone_animation->scale_vector[2]);
	
	//Rotate Blue Bone
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, glm::vec3({ 0.0f, 2.5f, 0.0f }));
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, glm::vec3({ 0.0f, -1.0f, 0.0f }));
	blue_bone_obj_mat = m_bone_animation->rotate_bone(blue_bone_obj_mat, m_bone_animation->rotation_degree_vector[3]);
	blue_bone_obj_mat = glm::translate(blue_bone_obj_mat, glm::vec3({ 0.0f, 1.0f, 0.0f }));
	blue_bone_obj_mat = glm::scale(blue_bone_obj_mat, m_bone_animation->scale_vector[3]);
