...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Rotate angle a", m_animation->m_angles[0])->setSpinnable(true);
	gui_1->addVariable("Rotate angle b", m_animation->m_angles[1])->setSpinnable(true);

	gui_1->addButton("Rotate Local X", [](){
		m_animation->rotateX(m_animation->m_angles[0]);
	});

	gui_1->addButton("Rotate Global Y", []() {
		m_animation->rotateY(m_animation->m_angles[1]);
	});

	gui_1->addButton("Reset Modal", []() {
		m_animation->reset();
	});
